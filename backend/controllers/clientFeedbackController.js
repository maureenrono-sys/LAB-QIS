...
exports.createFeedback = async (req, res) => {
    try {
        ...
        // Auto-link serious complaints and all suggestions to NC workflow.
        let autoNc = null;
        const shouldAutoCreateNc =
            (feedback.category === 'Complaint' && ['High', 'Critical'].includes(feedback.severity)) ||
            feedback.category === 'Suggestion';

        if (shouldAutoCreateNc) {
            const ncSeverity = feedback.category === 'Suggestion'
                ? 'Medium'
                : (feedback.severity === 'Critical' ? 'Critical' : 'High');
...
exports.linkToNc = async (req, res) => {
    try {
        ...
        const requestedNcId = sanitizeInput(req.body.ncId || '', 64);
        let nc = null;
        let autoGenerated = false;

        if (requestedNcId) {
            nc = await NonConformance.findOne({
                where: { id: requestedNcId, labId: req.user.labId }
            });
            ...
            nc = await NonConformance.create({
                labId: req.user.labId,
                description: `Auto-generated from feedback ${feedback.feedbackCode}: ${feedback.description}`,
                severity: ncSeverity,
                status: 'Open'
            ...
            await Notification.create({
                labId: req.user.labId,
                type: 'Update',
                message: `NC auto-generated and linked from feedback ${feedback.feedbackCode}.`
            });
        }
        ...
        await feedback.save();
        res.json({
            message: autoGenerated ? 'NC auto-generated and linked to feedback.' : 'Feedback linked to NC.',
            feedback,
            nc,
            autoGenerated
        });
